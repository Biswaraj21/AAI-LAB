...
class CarTypeDAG:
    ...
    def topological_order(self) -> list:
        ...
        if len(order) != len(self.tasks):
            raise ValueError(
                f"[{self.type_name}] DAG contains a cycle — "
...
class InputParser:
    ...
    _RE_CONFIG   = re.compile(
        r'^(\d+)\s+(\d+)\s+(\d+)(?:\s+(\d+))?$')
    _RE_CAR      = re.compile(
        r'^CAR\s+(\S+)\s+(\d+)$', re.IGNORECASE)
    _RE_TASK     = re.compile(
        r'^TASK\s+(\S+)(?:\s+->\s+(.+))?$', re.IGNORECASE)
    _RE_EDGE     = re.compile(
        r'(\S+):([0-9]*\.?[0-9]+)')
    ...
    def _parse_lines(self, lines):
        ...
        if not m:
            self._die(
                f"Line {lineno}: expected 'N  M  k  [seed]', got: {text!r}")
        N    = int(m.group(1))
        M    = int(m.group(2))
        k    = int(m.group(3))
        seed = int(m.group(4)) if m.group(4) else None
        for name, val in (("N (car types)", N),
                          ("M (mechanics)", M),
                          ("k (fatigue)", k)):
            if val < 1:
        ...
        while rest:
            ...
            if task_seen == 0:
                self._die(
                    f"CAR '{type_name}' has no TASK lines.")
...
class GarageScheduler:
    ...
    def __init__(self, M: int, k: int, rng: random.Random,
                 verbose: bool = False):
        self.M       = M
        self.k       = k
        self.rng     = rng
    ...
    def _assign(self, queue: deque):
        ...
        while queue:
            ...
            if ms.consecutive >= self.k:
                t_brk = len(row)
                row.append(ScheduleEntry(
                    time_unit=t_brk, mechanic_id=m, is_break=True))
                ms.consecutive   = 0
                ms.total_breaks += 1
                self._log(f"  {lbl} -> t={t_brk:>3}: [BREAK] "
                          f"(after {self.k} consecutive tasks)")

            t_slot = len(row)
            row.append(ScheduleEntry(
                time_unit=t_slot, mechanic_id=m, task=task))
            ms.consecutive  += 1
            ms.total_tasks  += 1
            spawn_tag = " [SPAWNED]" if task.is_spawn else ""
            self._log(f"  {lbl} -> t={t_slot:>3}: "
    ...
    def run(self, instances: list):
        ...
        while self.current_time < max_t:
            ...
            if new_tasks:
                ...
                self._log(
                    f"\n  SCHEDULE UPDATE: inserting "
                    f"{len(new_tasks)} sub-task(s)")
        ...
        for m in range(self.M):
            ...
            for edge in dag.children.get(task.task_id, []):
                ...
                if spawned:
                    ...
                    st = Task(task_id=child_id,
                              car_inst=task.car_inst,
                              is_spawn=True)
                    new_tasks.append(st)
                    self._log(
    ...
    def display_summary(self, instances: list):
        ...
        print("SUMMARY")
        print(f"{'=' * 70}")
        print(f"  Mechanics (M)        : {self.M}  "
              f"({', '.join(self.mech_labels)})")
        print(f"  Fatigue limit (k)    : {self.k}")
        print(f"  Cars serviced        : {len(instances)}  "
        ...
        print()
        print(f"  {'Mechanic':<10} {'Tasks':>6}  {'Breaks':>6}  "
              f"{'Utilisation':>12}")
        print(f"  {'-'*10} {'-'*6}  {'-'*6}  {'-'*12}")
        for m in range(self.M):
            ms   = self.mech_states[m]
            util = (ms.total_tasks / max_t * 100) if max_t else 0.0
            print(f"  {ms.label:<10} {ms.total_tasks:>6}  "
...
def main():
    ...
    ap.add_argument("--seed",   "-s", type=int, default=None, metavar="INT",
                    help="Override the random seed from the input file")
    ap.add_argument("--verbose","-v", action="store_true",
                    help="Log idle slots during simulation")
    args = ap.parse_args()
    ...
    print(f"  Mechanics (M)     : {M}  "
          f"({', '.join(f'M{i+1}' for i in range(M))})")
    ...
    scheduler.display_schedule(
        title="FINAL SCHEDULE GRID  (* = spawned sub-task)")
